function[] = battleRefresh(mapSize, introTrigger, playerHp, enemyHp, enemyHpOg, playerHpOg)
    ax = gca;
    zoom(ax, 'reset');
    set(ax, 'xlim', [0 mapSize*100]) %locks zoom?
    ...
    if playerHp > 0
        ...
        text((((mapSize*100)/1.3)),(mapSize*100-(mapSize*100)/2.1),'Player', 'Fontsize',18) %player name placeholder
    end
end